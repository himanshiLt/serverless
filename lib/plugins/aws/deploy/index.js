...
class AwsDeploy {
  constructor(serverless, options) {
    ...
    this.console = this.serverless.console;
    this.packagePath =
    ...
    this.hooks = {
      ...
      'aws:deploy:deploy:checkForChanges': async () => {
        ...
        // Activate console token, when console enabled
        if (this.console.isEnabled) {
          if (!this.state.console) {
            log.warning(
              'Cannot enable console: Service was packaged with disabled console configuration'
            );
            this.console.isEnabled = false;
            return;
          }
          if (this.state.console.schemaVersion !== this.console.stateSchemaVersion) {
            throw new ServerlessError(
              'Cannot deploy service: Service was packaged with different version of console integration',
              'CONSOLE_INTEGRATION_MISMATCH'
            );
          }
          if (this.state.console.orgId !== this.console.orgId) {
            throw new ServerlessError(
              'Cannot deploy service: Service was packaged in context of different console organization',
          ...
          this.console.overrideSettings({
            ...
            serviceId: this.state.console.serviceId,
          });
          await this.console.activateOtelIngestionToken();
        } else if (this.state.console) {
          throw new ServerlessError(
            'Cannot deploy service: Service was packaged with console integration, while console is ' +
              'currently diabled',
      ...
      'error': async () => {
        ...
        if (this.console.isEnabled) await this.console.deactivateOtelIngestionToken();
